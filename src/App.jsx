...
const DEADLINE   = new Date("2026-03-12T08:30:00Z");
...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .pk-hero-title {
    ...
    line-height:.9; color:#fff; text-transform:uppercase;
    margin-bottom:12px; position:relative; z-index:1; letter-spacing:-.01em;
  }
  .pk-hero-title span { color:${G.accent}; }
  .pk-hero-sub {
    font-size:14px; color:rgba(255,255,255,.55);
    margin-bottom:22px; position:relative; z-index:1; line-height:1.5;
  }
  .pk-deadline {
    display:inline-flex; align-items:center; gap:8px;
    background:rgba(232,93,4,.15); border:1px solid rgba(232,93,4,.35);
    border-radius:6px; padding:7px 16px;
    font-size:13px; font-weight:700; color:${G.accent};
    position:relative; z-index:1;
  }
  .pk-deadline.closed {
    background:rgba(220,38,38,.12); border-color:rgba(220,38,38,.3); color:${G.red};
  }
  ...
  .pk-step {
    ...
    color:${G.muted}; background:${G.surface};
    border-right:1px solid ${G.border}; transition:all .2s;
  }
  ...
  .pk-slot-num {
    font-family:'Barlow Condensed',sans-serif;
    font-size:20px; font-weight:900; color:${G.muted};
  ...
  .pk-team-btn.selected {
    border-color:${G.blue}; background:${G.blueDim};
  ...
  .pk-team-order {
    ...
    font-family:'Barlow Condensed',sans-serif;
    font-size:14px; font-weight:900; color:${G.accent};
    background:${G.accentDim}; border-radius:4px;
  ...
  .pk-input {
    ...
    font-size:15px; font-family:inherit; color:${G.text};
  ...
  .pk-submit-btn {
    ...
    font-family:'Barlow Condensed',sans-serif;
    font-size:17px; font-weight:900; letter-spacing:.08em; text-transform:uppercase;
  ...
  .pk-toast {
    ...
    border-radius:8px; z-index:999; white-space:nowrap;
  ...
  .pk-success {
    ...
    background:${G.surface}; border-radius:16px;
  ...
  .pk-success-title {
    ...
    color:${G.green}; margin-bottom:8px;
  }
  ...
  .pk-table th {
    background:${G.bg}; color:${G.muted};
    font-size:10px; font-weight:700; letter-spacing:.1em; text-transform:uppercase;
    padding:10px 14px; border-bottom:1.5px solid ${G.border}; text-align:left;
  }
  .pk-table td {
  ...
  .pk-banner.closed-banner { background:${G.redDim}; border:1px solid rgba(220,38,38,.25); color:#991b1b; }
  ...
  .pk-admin-title {
    ...
    color:${G.text}; margin-bottom:16px;
  ...
  .pk-admin-select {
    ...
    color:${G.text}; font-size:13px; font-family:inherit; outline:none; width:100%;
  ...
  .pk-btn {
    ...
    border-radius:8px; padding:10px 18px; cursor:pointer; transition:all .15s; border:1.5px solid;
  }
  ...
  .pk-count-input {
    width:70px; background:${G.bg}; border:1.5px solid ${G.border};
    border-radius:8px; padding:8px 10px;
    font-size:15px; font-family:inherit; color:${G.text};
    outline:none; text-align:center;
  }
  .pk-count-input:focus { border-color:${G.accent}; }
  .pk-selected-count { font-size:13px; color:${G.muted}; }
...
function logoUrl(filename, teamName) {
  // Use cargo image field, fall back to team name as filename
...
function logoFallback(e, teamName) {
  // If cargo image failed, try team name as filename
...
export default function App() {
  ...
  const [allWikiTeams, setAllWikiTeams] = useState([]); // all from wiki cargo
  const [wikiLoading, setWikiLoading]   = useState(false);
  const [wikiErr, setWikiErr]           = useState(null);

  // Active teams chosen by admin
  const [activeTeams, setActiveTeams]   = useState([]); // stored in Firestore meta
  ...
  const handleSubmit = async () => {
    if (!instaId.trim()) { showToast("Enter your Instagram ID", "error"); return; }
    if (picks.length < qualifyCount) { showToast(`Pick all ${qualifyCount} teams first`, "error"); return; }
    const cleanId = instaId.trim().replace(/^@/,"").toLowerCase();
    setSubmitting(true);
    try {
      const subsRef = collection(db, "pickem", META_DOC, SUBMISSIONS_COL);
      await setDoc(doc(subsRef, cleanId), {
        instaId: cleanId, picks, timestamp: new Date().toISOString(),
      });
      setSubmitted(true);
  ...
  const step = picks.length < qualifyCount ? 1 : !instaId.trim() ? 2 : 3;
      ...
      <div className="pk-root">
        ...
        <div className="pk-scoring">
          ...
          <div className="pk-scoring-item" style={{color:G.muted}}>
            Max {qualifyCount * 10} pts possible
          </div>
        </div>
            ...
            {activeTeams.length === 0 ? (
              ...
              <div className="pk-success">
                ...
                <div style={{color:G.textSub, fontSize:14, marginBottom:20}}>
                  @{instaId.replace(/^@/,"").toLowerCase()} · {picks.length} teams picked
                </div>
                ...
                <button className="pk-submit-btn" style={{maxWidth:260}}
                  onClick={()=>{setSubmitted(false);setPicks([]);setInstaId("");}}>
                  Edit Picks
                </button>
              </div>
            ) : isClosed ? (
              <div className="pk-loading">
                <div className="pk-banner closed-banner" style={{justifyContent:"center", textAlign:"center", display:"block", maxWidth:400, margin:"0 auto"}}>
              ...
              <>
                ...
                <div className="pk-section-label">
                  Your picks — {picks.length}/{qualifyCount}
                </div>
                ...
                {picks.length === qualifyCount && (
                  <div className="pk-form" style={{maxWidth:420}}>
                    <div className="pk-input-group">
                      <label className="pk-label">Instagram ID</label>
                      <input className="pk-input" placeholder="@yourusername"
                        value={instaId} onChange={e=>setInstaId(e.target.value)}
                        onKeyDown={e=>e.key==="Enter"&&handleSubmit()}/>
                    </div>
                    <button className="pk-submit-btn"
                      onClick={handleSubmit}
                      disabled={submitting || !instaId.trim()}>
                      {submitting ? "Submitting..." : "Submit Picks"}
                    </button>
                    <div className="pk-hint">
                      Resubmitting with the same Instagram ID overwrites your previous picks.
                    </div>
                  </div>
                )}
              </>
        ...
        {tab === "leaderboard" && (
          <div className="pk-container">
              ...
              <tbody>
                {scoredSubs.map((s,i) => (
                  <tr key={s.instaId}>
                    ...
                    <td style={{fontWeight:700}}>@{s.instaId}</td>
                    {published && <td style={{textAlign:"right", whiteSpace:"nowrap"}}><span className="pk-score-pill">{s.score} pts</span></td>}
                    <td>
            ...
            {published && (
              <div className="pk-legend" style={{marginTop:14}}>
                <div className="pk-legend-item">
                  <span className="pk-legend-dot" style={{background:G.green}}/>
                  Correct position (10 pts)
                </div>
                <div className="pk-legend-item">
                  <span className="pk-legend-dot" style={{background:G.blue}}/>
                  Correct team (5 pts)
                </div>
              </div>
            )}
              ...
              <>
                ...
                <div className="pk-admin-box">
                  ...
                  <div className="pk-count-row">
                    ...
                    <span className="pk-selected-count">
                      {adminSelectedTeams.length} of {allWikiTeams.length} teams selected
                    </span>
                  </div>
                  ...
                  {allWikiTeams.length > 0 && (
                    <>
                      <div className="pk-section-label" style={{marginBottom:10}}>
                      ...
                      <div className="pk-team-selector">
                        {allWikiTeams.map(t => {
                          ...
                          return (
                            <button key={t.team}
                              className={`pk-ts-btn${sel?" ts-selected":""}`}
                ...
                <div className="pk-admin-box">
                  ...
                  <div className="pk-btn-row">
                    <button className="pk-btn pk-btn-green" onClick={handleSaveResults}>
                      Save Results
                    </button>
                    {!published
                ...
                <div className="pk-admin-box">
                  ...
                  <table className="pk-table">
                    <thead>
                      <tr>
                        <th>#</th><th>Instagram</th><th>Submitted (IST)</th><th>Picks</th>
                      </tr>
                    </thead>
                    <tbody>
                      {[...submissions]
                      ...
                      {submissions.length===0 && (
                        <tr><td colSpan={4} style={{textAlign:"center",color:G.muted,padding:24}}>No submissions yet.</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
